...
function handleTouchStart(event) {
  ...
  touchActive = true;
  touchStartX = clientX - rect.left - rect.width / 2;
  touchStartY = clientY - rect.top - rect.height / 2;

  updateTouchIndicator(touchStartX, touchStartY);
  sendTouchData(touchStartX, touchStartY);
}
...
function handleTouchMove(event) {
  ...
  updateTouchIndicator(limitedX, limitedY);
  sendTouchData(limitedX, limitedY);
}
...
function handleTouchEnd(event) {
  ...
  fetch("/touch", {
    ...
    body: "x=0&y=0",
  }).catch((error) => console.error("Error releasing touch:", error));
}
...
function updateTouchIndicator(x, y) {
  ...
  touchIndicator.style.left = rect.width / 2 + x + "px";
  touchIndicator.style.top = rect.height / 2 + y + "px";
  touchIndicator.classList.add("active");
}

function sendTouchData(x, y) {
  const touchpad = document.getElementById("touchpad");
  const rect = touchpad.getBoundingClientRect();
  const maxDistance = rect.width / 2 - 10;
  ...
  fetch("/touch", {
    ...
    body: `x=${normalizedX.toFixed(3)}&y=${normalizedY.toFixed(3)}`,
  }).catch((error) => console.error("Error sending touch data:", error));
}

// Modify the existing enableManualControls function
function enableManualControls() {
  const emotionButtons = document.querySelectorAll(
    '.btn:not(.manual):not([onclick*="light()"])'
  );
...
// Modify the existing disableManualControls function
function disableManualControls() {
  const emotionButtons = document.querySelectorAll(
    '.btn:not(.manual):not([onclick*="light()"])'
  );
